...
namespace WebApiFaceCompare.Controllers
{
    ...
    public class FaceController : ControllerBase
    {
        ...
        public void TestCompare()
        {
            try
            {
                using LossMetric net = LossMetric.Deserialize("Resource/dlib_face_recognition_resnet_model_v1.dat");
                ...
                for (int i = 0; i < imgurl1.Length; i++)
                {
                    ...
                    var t1 = Task.Run(() =>
                    {
                        using ShapePredictor sp = ShapePredictor.Deserialize("Resource/shape_predictor_5_face_landmarks.dat");
                        using FrontalFaceDetector detector = Dlib.GetFrontalFaceDetector();
                        var req = WebRequest.CreateHttp(imgurl1[i]);
                        using Matrix<RgbPixel> img2 = ((Bitmap)Image.FromStream(req.GetResponse().GetResponseStream())).ToMatrix<RgbPixel>();
                        var dets2 = detector.Operator(img2);
                        foreach (var face in dets2)
                        {
                            using var shape = sp.Detect(img2, face);
                    ...
                    var t2 = Task.Run(() =>
                    {
                        ...
                        using (Matrix<RgbPixel> img21 = stre2.ToMatrix<RgbPixel>())
                        {
                            var dets21 = detector.Operator(img21);
                            foreach (var face1 in dets21)
                            {
                                using var shape1 = sp.Detect(img21, face1);
                                using var faceChipDetail1 = Dlib.GetFaceChipDetails(shape1, 150, 0.25);
        ...
        public string Compare(IFormFile image, string imgUrl = "https://gitee.com/laolaolulu/public/raw/master/20220920192624.jpg")
        {
            ...
            var t1 = Task.Run(() =>
            {
                ...
                {
                    using var shape = sp.Detect(img1, face);
                    using var faceChipDetail = Dlib.GetFaceChipDetails(shape, 150, 0.25);
